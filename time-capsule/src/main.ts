...
async function loadPublicCapsules(page = 1) {
  ...
  // 로그인 상태면 내 캡슐(user_id) 제외 → 다른 사람들 캡슐만 보임
  if (currentUser) {
...
export async function loadCapsules(publicPage = 1) {
  ...
  if (myError) {
    ...
    html += myCapsules.map(capsule => {
      ...
      return `
        <div class="capsule-card ${statusClass}" data-id="${capsule.id}">
          <div class="card-content">
  ...
  document.querySelectorAll('.open-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        modal.innerHTML = `
          <div style="background: white; border-radius: 24px; padding: 40px; max-width: 700px; width: 90%; max-height: 85vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.4); position: relative;">
            <button id="close-modal" style="position: absolute; top: 16px; right: 20px; background: none; border: none; font-size: 2.2rem; cursor: pointer; color: #666;">×</button>
            <h2 style="text-align: center; margin-bottom: 24px; color: #2c3e50; font-size: 2rem;">${capsule.title || '(제목 없음)'}</h2>
            <div style="white-space: pre-line; font-size: 1.15rem; line-height: 1.8; color: #333; margin-bottom: 32px;">
              ${decryptedContent}
            </div>
            <p style="text-align: center; color: #666; font-size: 1rem; margin-top: 32px;">
  ...
  document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      confirmModal.innerHTML = `
        <div style="background: white; border-radius: 20px; padding: 40px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,0.4);">
          <h2 style="color: #d32f2f; margin-bottom: 20px;">정말 삭제하시겠어요?</h2>
          <p style="color: #333; margin-bottom: 30px;">삭제된 캡슐은 복구할 수 없습니다.</p>
          <div style="display: flex; gap: 20px; justify-content: center;">
            <button id="cancel-delete" style="padding: 12px 24px; background: #ccc; color: white; border: none; border-radius: 12px; cursor: pointer;">취소</button>
            <button id="confirm-delete" style="padding: 12px 24px; background: #d32f2f; color: white; border: none; border-radius: 12px; cursor: pointer;">삭제</button>
          </div>
        </div>
      `;
